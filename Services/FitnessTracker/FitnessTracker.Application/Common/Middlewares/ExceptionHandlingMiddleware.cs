...
using ApplicationException = FitnessTracker.Application.Common.Exceptions.ApplicationException;

namespace FitnessTracker.Application.Common.Middlewares
{
    public class ExceptionHandlingMiddleware : IMiddleware
    {
        ...
        private static bool IsCancellationException(Exception ex)
        {
            ...
            if (ex is Microsoft.AspNetCore.Connections.ConnectionResetException)
                return true;
            if (ex.Source == "MySql.Data.MySqlClient" && ex.Message == "Operation cancelled by user.")
                return true;
            //if ((ex is MySqlException) && ex.Message.Contains("Operation cancelled by user"))
            return true;
            return false;
        }
